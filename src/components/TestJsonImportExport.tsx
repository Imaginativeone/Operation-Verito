...
const TestJsonImportExport: React.FC = () => {
  ...
  const testFullImportFromJson = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      myLog.current.push(
        "> createConnection " + " 'db-from-json' successful\n",
      );
      // open db "db-from-json"
      await db.open();
      myLog.current.push("> open " + " 'db-from-json' successful\n");
      ...
      myLog.current.push("> createSyncTable " + " 'db-from-json' successful\n");
      ...
      myLog.current.push("> getSyncDate " + " 'db-from-json' successful\n");
  ...
  const testPartialImportFromJson = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      myLog.current.push("> query " + " 'users' successful\n");
      ...
      myLog.current.push("> query " + " 'messages' successful\n");
      ...
      myLog.current.push("> query " + " 'images' successful\n");
      // close the connection
      await sqlite.closeConnection("db-from-json");
      myLog.current.push("> closeConnection successful\n");
   ...
   */
  const testFullExportToJson = useCallback(
    async (db: SQLiteDBConnection): Promise<boolean> => {
   ...
   */
  const testPartialExportToJson = useCallback(
    async (db: SQLiteDBConnection): Promise<boolean> => {
